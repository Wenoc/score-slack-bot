.PHONY: help
AWS_STACK_NAME=score-slack-bot
AWS_S3_BUCKET=score-slack-bot
AWS_ACCOUNT=scm-tori-pro
...
zip: # builds the linux binary, and creates the zip for lambda upload
	GOOS=linux GOARCH=amd64 go build -o main  -ldflags '-X main.Version="$(VERSION)"' main.go aws.go
	zip main.zip main
